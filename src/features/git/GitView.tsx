...
interface GitViewProps {
  fs: any;
  width: number;
  onOpenFile: (path: string, options?: { isDiff?: boolean, diffOriginalContent?: string }) => void;
}
    ...
    <div class="h-full w-full flex flex-col p-3 overflow-y-auto" style={{ color: "var(--color-fg)" }}>
      ...
      >
        ...
        <div class="flex flex-col gap-4">
          <Show when={git.gitState()?.status.filter(s => s.status.startsWith("A") || s.status.startsWith("M") || s.status.startsWith("D") || s.status.startsWith("R")).length! > 0}>
            <div class="flex flex-col gap-1">
              <span class="text-[11px] font-semibold uppercase px-1" style={{ color: "var(--color-fg-muted)" }}>Staged Changes</span>
              <For each={git.gitState()?.status.filter(s => s.status[0] !== ' ' && s.status[0] !== '?')}>
                {(file) => (
                  <GitFileItem
                    file={file.file}
                    status={file.status}
                    onClick={async () => {
                      ...
                      const isConflicted = file.status.includes("U") || file.status === "DD" || file.status === "AA";
                      if (isUntracked || isConflicted) {
                        props.onOpenFile(`${props.fs.rootPath()}/${file.file}`);
                      } else {
                    ...
                    actions={[
                      {
                        ...
                        onClick: (e) => { e.stopPropagation(); git.unstageFiles([file.file]); }
                      }
                    ]}
                  />
                )}
          ...
          <div class="flex flex-col gap-1">
            <div class="flex items-center justify-between group h-[22px]">
              <span class="text-[11px] font-semibold uppercase px-1" style={{ color: "var(--color-fg-muted)" }}>Changes</span>
              <Show when={git.gitState()?.status.filter(s => s.status[1] !== ' ').length! > 0}>
                <button 
                  class="opacity-0 group-hover:opacity-100 p-1 hover:text-[var(--color-success)] transition-opacity" 
                  onClick={() => git.stageFiles(git.gitState()?.status.filter(s => s.status[1] !== ' ' || s.status === "??").map(f => f.file) || [])}
            ...
            <Show
              when={git.gitState()?.status.filter(s => s.status[1] !== ' ').length! > 0}
              fallback={<div class="px-2 text-xs" style={{ color: "var(--color-fg-muted)" }}>No changes</div>}
            >
              <For each={git.gitState()?.status.filter(s => s.status[1] !== ' ' || s.status === "??")}>
                {(file) => (
                  <GitFileItem
                    file={file.file}
                    status={file.status}
                    ...
                    actions={[
                      {
                        ...
                        onClick: async (e) => {
                          e.stopPropagation(); 
                          if (file.status === "??") {
                            const confirmed = await ui.showConfirmDialog(`Are you sure you want to delete ${file.file.split(/[\\/]/).pop()}?\nThis action is irreversible!`, { variant: "danger", okLabel: "Delete File" });
                            if (confirmed) {
                              try {
                                await remove(`${props.fs.rootPath()}/${file.file}`);
                                git.refreshGit();
                              } catch (err) {
                      ...
                      },
                      {
                        ...
                        onClick: (e) => { e.stopPropagation(); git.stageFiles([file.file]); }
                      }
                    ]}
                  />
                )}
